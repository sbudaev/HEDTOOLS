...
optional integer parameter +iostat+ returns the error status of the operation,
0 in case of success, -1 if there was an error creating the directory.

The error -1 is returned also if there is already a directory with the same
...
useful, for example, in case when one need to create a directory, but the
