...
- *Working copy:* the local file system directory that keeps the files
  synced with the Subversion server (_your copy_).
  indexterm:[working copy]

- *Checkout:* download the files from the Subversion server _initially,_ this
  sets up all the necessary data within the chosen *working copy*. Checkout is
  done _only once_.
  indexterm:[checkout]
...
- *Update:* get the files with the latest changes from the Subversion server
  ...
  locally are kept intact. To cancel all local changes use *revert*.
  indexterm:[update]
