...
Latest PDF version: +
http://129.177.185.19/doc/HEDTOOLS.pdf
...
IMPORTANT: A Code::Blocks project template for the AHA Model is available
    ...
    https://svn.uib.no/aha-fortran/trunk/scripts/Project.cbp[Project.cbp].
...
(http://docs.uh-iaas.no/en/latest/[IaaS]). There is a virtual workstation for
developing the AHA Model based on this cloud infrastructure. The "virtual
developer 's computer" is based on the Debian Linux OS and has all the
essential software pre-installed and ready for use out of the box.

IMPORTANT: The current system is based on GNU gfortran and
           <<CODEBLOCKS_USE, Code::Blocks IDE>>.

indexterm:[IaaS]
indexterm:[AHA Workshop, cloud-based workstation]

To connect the cloud-based AHA Workshop you need the *x2go* client software for
the local physical computer that is used to access the cloud workstation. Once
connected, it works just like a physical Linux computer preloaded with all the
<<FORTRAN_GET, required software>>:
...
On the Ubuntu Linux installation is by command line:
...
IMPORTANT: If you have got an ssh key file (.pem), it should be kept secret
           ...
           is the only method to get access to the cloud AHA Workshop workstation
           (there is no password).

The user name at the AHA Workshop workstation is *debian*. To connect, you
will also be given an unique ip address. The session type for the Linux machine
is always XFCE. Xfce is a simple and lightweight desktop environment. Check out
its official web site: https://www.xfce.org/ .

A typical setup of the AHA Workshop is like this:
...
It is also possible to connect the remote system via plain ssh:
