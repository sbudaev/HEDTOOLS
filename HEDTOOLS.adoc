...
Getting the <<SVN_INTRO, Subversion>> branch is simple, it can be done either
on the Terminal command line or using the simple Subversion client
<<SVN_INTEGR, integrated>> into the default _Thunar_ file manager.

image:doc/img/img_doc_cloud_svn1.png[width=400,align="left"]

The <<CODEBLOCKS_USE, Code::Block>> installation on the AHA Workshop machines
has a simple Subversion integrated into the tools menu. This allows to run
basic elementary commands like `update`, `diff`, `commit` and `reverse`.

image:doc/img/img_doc_cloud_cb_tools.png[width=400,align="left"]
