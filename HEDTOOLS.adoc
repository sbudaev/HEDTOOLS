...
$Id: HEDTOOLS.adoc 19412 2025-10-14 06:39:16Z sbu062 $
:toc:
...
*Lead zeroes.* With integers, TOSTR can also generate leading zeros, which
is useful for auto-generating file names or variable names. In such cases,
the number of leading zeros is determined by the second optional *integer*
parameter. This integer sets the template for the leading zeros, the maximum
string. The exact value is unimportant, only the number of digits is used.
...
*Verbatim format spaces.*  With scalar integer and real arguments, TOSTR
can also retain leading spaces, when called with the third logical argument
`is_clean`, this really makes sense when the format string specifier is used,
then verbatim format specifier is used for the output .  When `is_clean`
is set to TRUE or unset (default), the function output string is cleaned of
blanks, if it is FALSE, no cleanup is done. For example:

[source]
--------------------------------------------------------------------------------
">>" // TOSTR( Output_Stats%total_vol_evacuated, "(f10.4)" ) // "<<"
! This code produces >>1.8169<<
--------------------------------------------------------------------------------

[source]
--------------------------------------------------------------------------------
">>" // TOSTR( Output_Stats%total_vol_evacuated, "(f10.4)", .FALSE. ) // "<<"
! produces >>    1.8169<< (verbatim format, leading blanks are not deleted)
--------------------------------------------------------------------------------
...
The code below illustrates how the whole process
...
! WARNING: the record string variable cannot be allocatable
...
This results in the following data file output:
...
Additionally, there is a wrapper for saving the whole chunk of the data at once.
...
Note that the string value is by default quoted in the output CSV file. To
avoid quoting, use optional logical parameter `is_quoted` set to `FALSE`. For
example

[source]
--------------------------------------------------------------------------------
! Here a numeric variable ratio is first converted to a formatted string and
! then appended to a CSV record without quoting. This can be a method to save
! a numeric value forcing with a specific format.
call CSV_RECORD_APPEND(record_csv, TOSTR(ratio,FFMT),is_quoted=.FALSE.)
--------------------------------------------------------------------------------
...
+FUNCTION MATCH(str, ipos)+
indexterm:[MATCH]

This function returns the delimiter in string +str+ that matches the delimiter
in position +ipos+ of +str+. The output is the position of the matching
delimiter or -1 in case of error (e.g. no matching delimiter).  Allowable
delimiters are (), [], {}, <>.
