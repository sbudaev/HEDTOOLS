...
The subroutines +RAND_ARRAY+ and +RNORM_ARRAY+ generate arrays (up to
6-dimensions) of uniform and gaussian random numbers. The only parameter of
these subroutines is the +real+ +(kind=4)+ array to fill random numbers with.
There are also aliases +RAND_MATRIX+ and +RNORM_MATRIX+ for two- to
...
IMPORTANT: The commands in the Makefile must start from the +TAB+ symbol.
...
The start of the Makefile contains several variables defining basic parameters
of the build process. For example, here it defines the three supported compilers
(e.g. +GF_FC+ GNU gfortran) and sets the default compiler that is used is +make+
...
The variable SRC defines the main source code file(s) for the model (without
HEDTOOLS).
...
This variable can define several files, e.g.
...
or wildcards for files in the current directory:
...
* +SRC+ is the source file name(s) for the model,
