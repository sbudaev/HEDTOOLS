...
Using Intel Parallel Studio on Windows
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Intel Parallel Studio environment can be inserted in the *Startup -> Tasks ->
Commands* menu:

indexterm:[ConEmu terminal]

image:img_doc_intel_win_conemu.png[height=245,align="left"]

Then, all the Intel build environment commands will be run automatically every
time the +ConEmu+ terminal is started, so you would not need to do this
manually.
