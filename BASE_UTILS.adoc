...
image:doc/img/ok.svg[width=16] Normal *variables* should be in +lowercase+,
global and local *parameters* that are not allowed to change, in
...
than a single screen page (with vertical orientation). So the whole bunch of
code is easy to overview and work with. "Atomic" operations are particularly
helpful in the object oriented code.
