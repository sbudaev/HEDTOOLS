...
Document code on the go with Doxygen
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Thus, a trace of the execution can be read by the user during and after the execution. The module includes subroutines to
...
The logger must be started up with the subroutine +LOG_STARTUP+ that has
the log file name as a parameter.
...
The last thing to do is to shutdown logging with the +LOG_SHUTDOWN+ subroutine.
