...
the executables. They are subsequently called from the main model code using
the standard intrinsic Fortran procedures to run the system (shell) commands:
+execute_command_line+ or (older +system+).

Here is an example of calling the scatterplot using a wrapper (+call_external+)
to the intrinsic +execute_command_line+ subroutine.
...
!! @note Note that we set background task as FALSE if removing CSV file
!!       to avoid removing data before plot is produced.
call call_external(                                                       &
