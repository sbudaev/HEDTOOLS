...
directory (i.e. does not require administrative rights).
...
image:img_doc_ms_studio_solution.png[width=510,align="left"]
...
image:img_doc_ms_studio_fail.png[width=510,align="left"]
...
image:img_doc_ms_studio_ignore.png[width=510,align="left"]
...
Working with the <<CODEBLOCKS_GET, Code::Blocks IDE>> is similar to that in
Microsoft Studio. The simplest way to get it running is to (1) build HEDTOOLS
using the make system, (2) copy all the static library +lib_hedutils.a+ (or
+lib_hedutils.lib+) and all the +.mod+ files produced by the make build into
the working project directory, add the library file to the project file using
the *Project -> Add Files* menu. After this it should build within the IDE.

indexterm:[Code::Blocks]

image:img_doc_cb_add_lib.png[width=510,align="left"]

When building, a link order issue may appear, but this does not affect anything,
so in most cases just ignore it.

image:img_doc_cb_link_order.png[width=314,align="left"]
...
image:img_doc_cb_relative.png[width=400,align="left"]
