...
others:
    ...
    Fortran Evolutiuonary Agents Simulation Toolbox (FEAST)
...
prompt setup script +ipsxe-comp-vars.bat+ (with full path) and then run the
Far Manager (also full path is here).
...
started), so you would not need to do this manually.
...
*This is a difficult but flexible method.*
...
*This is the easiest method not requiring any manual tweaking.*
...
with +FC=ifort+, this is the default +make+ target:
...
second, links the code with the library.
