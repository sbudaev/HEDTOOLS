...
program LOOP_FREE
  ...
  implicit none
  character(len=*), parameter :: fmt_str_r = "(3F8.1)"
  character(len=*), parameter :: fmt_str_i = "(3I8)"
...
! *** Example 2: Calculate sums of elements over the fixed (second) dimension
  S = sum(A, dim=2)
  print *, "---------------------------"
  print (fmt_str_i), S       !       5       9     400

! *** Example 3: Find if the condition holds, for the fixed (second) dimension.
  AB = all(A > B, dim=2)
  print *, AB                ! F F T
...
order, such as *loop-free array constructions,* that works fast just everywhere.
