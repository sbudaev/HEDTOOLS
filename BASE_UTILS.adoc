...
Subroutine: ARRAY_IDRANK
~~~~~~~~~~~~~~~~~~~~~~
indexterm:[ARRAY_IDRANK]

*ARRAY_IDRANK* calculates an integer index array for sorting a real (kind 4 or
8) or integer array. The first parameter defines the array to be sorted, the
second is the integer indexing array  into an index array, using merge-sort.

Example:

[source]
--------------------------------------------------------------------------------
call ARRAY_IDRANK(array_to_rank, index_array)
! To get a sorted array, just use this index array: index_array
array_sorted = array_to_rank(index_array)
--------------------------------------------------------------------------------
