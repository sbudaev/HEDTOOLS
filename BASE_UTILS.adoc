...
[source]
--------------------------------------------------------------------------------
!> The module **COMMONDATA** is used for definine various global parameters like
!! model name, tags, population size etc.
module COMMONDATA
  ...
  !> Model description - a longer descriptive text, used in text outputs etc.
  character (len=*), parameter, public :: MODEL_DESCR= &
    "Prototype object-oriented model with genetic algorithm."
--------------------------------------------------------------------------------

There are various options and keywords. A few of them are very frequently
used in modelling:
...
function TAG_MMDD() result (MMDD)
    implicit none
    !> @returns MMDD Returns an 8-character string for YYYYMMDD
    character(8) MMDD
--------------------------------------------------------------------------------

Doxygen parses the source code and produces highly structured documentation
in different formats (html, rtf, latex, pdf):

image:img_doc_doxygen.png[width=490,align="left"]

Documenting a complex model is very important! And Doxygen allows inclusion
of various markup commands and styles, LaTeX formulas and graphics. Doxygen
documentation and howtos is available here: http://doxygen.org
