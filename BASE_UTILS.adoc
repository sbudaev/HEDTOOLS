...
Subversion has a very useful feature: you can set various properties
(+svn propset+). For example, one can set tags on files or directories. A very
interesting feature is that  properties can be incorporated into the source
files under SVN control. For example, you can include specific tags into
the Fortran (or any other managed) source code so that they are updated
automatically.
...
Check out full documentation in the SVN manual about +propset+.

GUI Tools
~~~~~~~~~

Using the GUI tools like <<SVN_GET, TortoiseSVN>> is similar. With GUI you
should just select the appropriate item from the menu list.

image:img_doc_tortoise.png[height=200,align="left"]

Similar GUI tools exist for Linux. For example, there is +thunar-vcs-plugin+
(Git and Subversion integration into the
