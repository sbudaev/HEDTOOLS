...
The +log+ command will issue the list of log messages, by default in reverse
order (the most recent logs are the first), so the development progress is seen.
*Example:* To show only +5+ most recent log messages for a specific file use
such a command:
...
There is a small *caveat* with +svn log+: by default it shows log messages from
the local _working copy_ (not repository). So, if you did many commits lately but
did not +svn update+, the latest messages will be absent from the log. So,
*do +svn update+!*
