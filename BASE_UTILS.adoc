...
Subroutines: ARRAY_IDRANK and ARRAY_RANK
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
indexterm:[ARRAY_IDRANK]
indexterm:[ARRAY_RANK]
indexterm:[sorting index]

*ARRAY_IDRANK* calculates an integer sorting index vector for sorting a
real (kind 4 or 8) or an integer input vector. The first parameter of the
...
call ARRAY_IDRANK(X, index_array)
...
using the integer sorting index vector that is calculated by +ARRAY_IDRANK+.
indexterm:[ARRAY_RANK]
...
This shows how to use +ARRAY_IDRANK+ and +ARRAY_RANK+ together.
...
program TESTRANKS
  ...
  call ARRAY_IDRANK(X, Id)   ! Calculate index vector for X:
