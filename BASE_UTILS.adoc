...
The code below reshapes these two vectors into a single two-column matrix (by
columns), so in the final CSV file they represent two separate variables. Note
that there is also an array of two descriptive column names of the same
string length.
...
And because we do not use several loops to combine the values manually, 
automatic reshaping of the arrays is (usually) very fast, in many cases could 
be automatically pareallelised.
